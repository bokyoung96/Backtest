...
# NOTE: 3, 6, 12-month momentum with 2, 1, -1 weights


class MethodologyMomentumComb(Methodology):
    def __init__(self,
                 mkt: str = 'KOSPI200',
        ...
        self.lookback = lookback if isinstance(lookback, list) else [lookback]
        self.lookback_weights = lookback_weights if isinstance(lookback_weights, list) else [1] * len(self.lookback)
        
        if len(self.lookback) != len(self.lookback_weights):
            raise ValueError("lookback and lookback_weights must have same length")
    ...
    def get_raw_factor(self):
        try:
            ...
            for i, (lookback_period, weight) in enumerate(zip(self.lookback, self.lookback_weights)):
                momentum = month_end_df.pct_change(periods=lookback_period, 
                                                 fill_method=None)
                momentum = momentum.where(np.isfinite(momentum), 
                                        np.nan).infer_objects(copy=False)
                
                momentum_aligned = momentum.reindex(month_periods).set_index(orig_idx)
                if i == 0:
                    raw_factor = momentum_aligned * weight
                else:
                    raw_factor += momentum_aligned * weight
            return raw_factor
...
if __name__ == "__main__":
    m = MethodologyMomentumComb(mkt='KOSPI200',
                                start_date='20110101',
                                end_date='20250101',
                                quantile=5,
                                quantile_position=[1],
