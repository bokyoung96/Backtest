...
class PreProcess:
    ...
    def lag_data(self,
        ...
        df.index = pd.DatetimeIndex(
            [item['available_date'] for item in availability_info])
        df = df.sort_index()

        combined_idx = orig_idx.union(df.index).sort_values()

        lagged_data = df.reindex(
            combined_idx, method='ffill').reindex(orig_idx)
        return lagged_data
            ...
            if hasattr(self, method):
                ...
                if data is not None and not data.empty:
                    output_path = os.path.join(
                        self.data_dir, f"{self.file_name}.parquet")
...
#     pp.save_data(method="get_QW_res_data", lag_months=[3,3,3,4], remove_strings=True, calculate_ttm=True)
